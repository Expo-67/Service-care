...
import gti from "../assets/gti.jpeg"; // Add your logo image here
...
const Page = () => {
  ...
  // Effect hook to clear error message on field change
  useEffect(() => {
    setError("");
  }, [name, email, password]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/auth/signup`, // Signup endpoint
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({ name, email, password }), // Send name, email, and password for signup
        }
      );
      ...
      if (response.ok) {
        ...
        // Store the token in localStorage (or cookie for secure storage)
        localStorage.setItem("token", data.token);
        // Redirect to dashboard upon successful signup
        router.push("/login");
      } else {
        const errorData = await response.json();
        setError(errorData.message || "Error during signup");
      }
    } catch (err) {
