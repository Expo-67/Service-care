...
export const signup = async (req, res) => {
  ...
  try {
    if (!email || !password || !name) {
      throw new Error("All fields are required"); //Parse an error if the following data is not matching
    }
...
//Reset password password here
export const resetPassword = async (req, res) => {
  try {
    const { token } = req.params; //the token is what we have called in the routes
